...
BEGIN
    ...
    -- We don't use INTO STRICT here because that error (no column) is surfaced
    -- later. We only care if this is INTEGER, all other types (including no
    -- type) can continue as before.
    SELECT atttypid
    INTO time_type
    FROM pg_attribute
